...
const fileStructure = {
    "Projects": {
        ...
        "Tune Tracer": {
            files: [
                { name: "summary.txt", summaryKey: "tune-tracer"},
        ...
        "Undergrad Research": {
            files: [
                { name: "summary.txt", summaryKey: "undergrad"},
        ...
        "Room Raiser":{
            files: [
                { name: "summary.txt", summaryKey: "room-raiser"},
...
const FileExplorer = forwardRef(({ onClose, zIndex, onFocus }, ref) => {
    ...
    const isText = selectedMedia?.name?.match(/\.(txt)$/i) || !!selectedMedia?.content;
                    ...
                    <div className="files">
                        <AnimatePresence>
                            {currentFolder.files.map((file, index) => {
                                const isTextFile = file.name.match(/\.txt$/i);
                                const isVideoFile = file.name.match(/\.(mp4|webm|ogg|mov)$/i);

                                let mediaIcon = "imagefile-icon.png";
                                if(isTextFile) {
                                    mediaIcon = "textfile-icon.png";
                                } else if (isVideoFile) {
                                    mediaIcon = "videofile-icon.png";
                                }
                                ...
                                return (
                                    <motion.div
                                        ...
                                        animate = {{ scale: 1}}
                                        exit = {{ scale: 0}}
                                        transition={{ duration: .75, bounce: .5, type: "spring", ease:"easeIn"}}
                                        onClick={() => openImage(file)}
                                    >
                                        <img src={mediaIcon} alt={file.name} />
                                        <span>{file.name}</span>
                                    </motion.div>
                                    );
                                })}
                    ...
                    <div className="lightbox" onClick={() => setselectedMedia(null)}>
                        ...
                        {isText ? (
                            <div className="summary-text"
                                dangerouslySetInnerHTML = {{ __html: textSummaries  [selectedMedia.summaryKey]}}
